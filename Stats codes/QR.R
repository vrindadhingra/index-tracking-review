qr_reg<-function(indata,indexin, tau, k)
{
  params <- list()
  ...
  #total number of variables: [1:n] (portfolio weights), [n+1:2n] card (binary) variables, [2n+1]: alpha_opt(d), [2n+2]: beta_opt(e)
  m=2*n+2
  ...
  for(j in 1:n)
  {
    reg_qr=rq(AR[,j]~IR, tau=tau)
    alpha[j]=reg_qr$coefficients[1]
    beta[j]=reg_qr$coefficients[2]
  }
  ...
  Amat1=rbind(A,B,C,D1,D2)
  
  #Step1: Minimizing Alpha
  model1$obj   <- c(rep(0,n), rep(0,n), 1, 0)
  model1$A     <- Amat1
  model1$rhs   <- c(1,k,rep(0,n), 0, 0)
