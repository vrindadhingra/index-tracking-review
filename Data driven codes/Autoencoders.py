"""
Created on Wed Jan  8 20:27:05 2025
...
def portfolio_selection(index_returns, asset_returns, ranked_assets, k, reg_lambda=1e-4):
    """
...
# Load Data
file_path = "/....enter path to load the price data.../sp500_daily.xlsx"
price_data = pd.read_excel(file_path)

# Convert to matrix and remove any non-numeric columns (e.g., dates)
price_matrix = price_data.iloc[:, 1:].values  # Exclude the first column (Date)

# Convert to numeric matrix (if not already numeric)
...
price_row = log_prices.shape[0] - 1
price_col = log_prices.shape[1]

# Index price series (log prices)
ind_log_price = log_prices[:, 0]

# Asset log price series
ast_log_price = log_prices[:, 1:]

# Number of assets and scenarios
N = ast_log_price.shape[1]  # Number of assets
T = ast_log_price.shape[0]  # Number of scenarios
...
for i in range(win_count):
    m1 = roll * i
    m2 = train_len + roll * i
    IR = ind_log_price[m1:m2]
    AR = ast_log_price[m1:m2, :]
    k = 45
